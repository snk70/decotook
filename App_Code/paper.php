...
<head>
    <title>مطالب دکوراسیون داخلی</title>

<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Language" content="fa">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<META NAME="Description" CONTENT="دکوتوک مرجع دکوراسیون داخلی منزل , ترفند زیبا سازی منازل , مطالب مرتبط با دکوراسیون ">
<META NAME="Keywords" CONTENT="دکوتوک , مطالب دکوراسیون داخلی , دکوراسیون منزل , دکوراسیون">
  ...
  <h1 class="pt3 fs16">برای یافتن مطالب مورد نظر در حوزه دکوراسیون داخلی ، میتوانید ادسته مورد نظر را از منوی کناری انتخاب نمایید</h1>
        ...
        <?php
					...
					mysql_query('SELECT * FROM `tbl_papers`');
	...
	if(isset($_GET["search_v"]) || isset($_GET["sort_v"]))
	{
			if(isset($_GET["search_v"]) && isset($_GET["sort_v"])==False)
			{
				...
				$rows2=mysql_query('SELECT * FROM `tbl_papers` WHERE (cate_title LIKE \''.$_GET["search_v"].'%\' OR cate_title LIKE \'%'.$_GET["search_v"].'\')');
		...
		$rows2=	mysql_query('SELECT * FROM `tbl_papers` WHERE(r>'.$sort_cat_1.' AND r<='.$sort_cat_2.')');
	...
	}else
	{
		...
		$rows2=	mysql_query('SELECT * FROM `tbl_papers` WHERE(r>0 AND r<=10)');
		...
		{
		$row3=mysql_query('SELECT (r) FROM `tbl_papers`');
