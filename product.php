...
<head>
    <title>فروشگاه آنلاین لوازم ساختمانی و کالاهای دکوراتیو</title>

<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta http-equiv="Content-Language" content="fa">
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    <meta name="description" content="دکوتوک فروش آن لاین محصولات ساختمانی و کالاهای دکوراتیو, معرفی خدمات معتبر نقاشی, کاغذ دیواری و تعمیرات ساختمانی, طراحی و اجرای کناف, طراحی و اجرای دیوارهای آکوستیک (عایق صدا) ویژه آپارتمان و خانه">
    <meta name="keywords" content="فروشگاه آن لاین محصولات دکوراتیو, فروش آن‏لاین محصولات ساختمانی, درخواست آن لاین برای بازسازی و تعمیرات ساختمان">
  ...
  <body class="bg_Kerem">
	...
	<!--/#portfolio-item-->
		...
		if(isset($_GET["search_v"]))
		{
					...
					$row=mysql_query('SELECT * FROM `tbl_cat_prd` WHERE (search_value=\''.$_GET["search_v"].'\')',$link);
					$r=mysql_fetch_assoc($row);
			
			echo('<a href="index.php" class="fl_r pt3">صفحه اصلی&ensp;</a><a class="fl_r pt3">/&ensp;</a><a href="product.php" class="fl_r pt3">محصولات و خدمات</a><a class="fl_r pt3">/&ensp;</a><a class="pt3 fl_r">'.$r["text"].'&ensp;</a><br>');
		}else
		{
			echo('<a href="index.php" class="fl_r pt3">صفحه اصلی&ensp;</a><a class="fl_r pt3">/&ensp;</a><a href="product.php" class="fl_r pt3">محصولات و خدمات</a><br>');
		}
		?>
        
  <p class="pt1 mrg_top_20">محصولات و خدمات</p>
  <h1 class="pt3 fs16">برای یافتن محصولات و خدمات مورد نظر در حوزه دکوراسیون داخلی ، میتوانید دسته مورد نظر را از منوی کناری انتخاب نمایید</h1>
        <hr class="h_height2">
	...
	if(isset($_GET["search_v"]) || isset($_GET["sort_v"]))
	{
			if(isset($_GET["search_v"]) && isset($_GET["sort_v"])==False)
			{
				...
				$rows2=mysql_query('SELECT * FROM `tbl_products` WHERE (cate_title LIKE \'%\' \''.$_GET['search_v'].'\' \'%\') ORDER BY `r` DESC',$link);
				$lvl=0;
			...
			{
				$rows2=mysql_query('SELECT * FROM `tbl_products` ORDER BY `r` DESC',$link);
				$lvl=0;
				while($r2=mysql_fetch_assoc($rows2))
				{
					$lvl++;
			...
			if(isset($_GET["sort_v"]) && isset($_GET["search_v"]))
			{
				...
				$rows2=	mysql_query('SELECT * FROM `tbl_products` WHERE (cate_title LIKE \''.$_GET["search_v"].'%\' OR cate_title LIKE \'%'.$_GET["search_v"].'\' ) ORDER BY `r` DESC');
	...
	}else
	{
		...
		if(isset($_GET["search_v"]))
		{
			$row=mysql_query('SELECT COUNT(r) FROM `tbl_products` WHERE (cate_title LIKE \'%\' \''.$_GET["search_v"].'\' \'%\')',$link);
		}else
		{
		$row=mysql_query('SELECT COUNT(r) FROM `tbl_products`',$link);
		}
