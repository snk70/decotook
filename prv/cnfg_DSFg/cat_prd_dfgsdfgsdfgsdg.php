...
<p class="fl_r">ویرایش دسته های محصولات</p><br><br><br>

<a href="cat_dfkgsdkfjjksdfgsdfjg.php" class="fl_r"><button class="fl_r">رفتن به صفحه دسته مطالب</button></a>
...
if(isset($_GET["opr"]))
{
	...
	if($_GET["opr"]=="del")
	{
		...
		mysql_query('DELETE FROM `tbl_cat_prd` WHERE `r` = '.$_GET["r"]);
	...
	}elseif($_GET["opr"]=="ed")
	{
					...
					$rows2 = mysql_query('SELECT * FROM `tbl_cat_prd` WHERE (r=\''.$_GET["r"].'\')');
	...
	}elseif($_GET["opr"]=="up")
	{
					...
					mysql_query("UPDATE `tbl_cat_prd` SET `r` = '".$_GET["r"]."', `text` = '".$_GET["text"]."', `search_value` = '".$_GET["search_value"]."', `sub_cat` = '".$_GET["sub_cat"]."' WHERE (`r` = ".$_GET["r0"].")");
	...
	}elseif($_GET["opr"]=="add")
	{
					...
					mysql_query("INSERT INTO `tbl_cat_prd` (`r`, `text`, `search_value`, `sub_cat`) VALUES (NULL, '".$_GET["text"]."', '".$_GET["search_value"]."', '".$_GET["sub_cat"]."')");
...
<?php
					...
					mysql_query("SET CHARACTER SET 'utf8'", $link);
					mysql_query("SET character_set_connection = 'utf8'", $link);
					$rows = mysql_query('SELECT * FROM `tbl_cat_prd`');
